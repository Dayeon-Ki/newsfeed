  ...
  try {
    ...
    if (existingUser) {
      ...
      if (nickname === existingUser.nickname)
        return res.status(400).json({ errMessage: '이미 존재하는 닉네임입니다.' });
...
router.delete('/:userId', auth, async (req, res) => {
  ...
  if (!user) return res.status(400).json({ errMessage: '잘못된 접근입니다. 존재하지 않는 회원입니다.' });
  if (user) {
    if (userId !== user.userId) {
      return res.status(400).json({ errMessage: '본인만이 삭제할 수 있습니다.' });
    } else {
